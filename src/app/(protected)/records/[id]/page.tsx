        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="bg-white dark:bg-neutral-900 rounded-xl p-6">
            ...
            {Array.from({ length: 4 }).map((_, j) => (
              <div key={j} className="h-4 bg-slate-100 dark:bg-neutral-900 rounded animate-pulse mb-2" />
            ))}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black py-10 px-4 text-slate-900 dark:text-slate-100">
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* Medical Data */}
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-xl shadow fade-in fade-in-2">
          ...
          <div className="divide-y divide-slate-100 dark:divide-neutral-800">
            {[
              ...
              <div key={label} className="flex gap-4 py-2.5">
                <span className="text-sm text-slate-500 dark:text-neutral-400 w-32 shrink-0">{label}</span>
                <span className="text-sm text-slate-900 dark:text-white font-medium">{value}</span>
              </div>
            ))}
            <div className="flex gap-4 py-2.5">
              <span className="text-sm text-slate-500 dark:text-neutral-400 w-32 shrink-0">Severity</span>
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${severityColors[record.severity] || "bg-slate-100 text-slate-600"}`}>
                {record.severity}
              </span>
            </div>
            {[
        ...
        {/* Verification */}
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-xl shadow fade-in fade-in-3">
          <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-3">Integrity Verification</h2>
          <div className="divide-y divide-slate-100 dark:divide-neutral-800">
            <div className="flex gap-4 py-2.5">
              <span className="text-sm text-slate-500 dark:text-neutral-400 w-32 shrink-0">Status</span>
              {isValid === null ? (
                <span className="text-sm text-slate-500 dark:text-neutral-400">Checking...</span>
              ) : isValid ? (
                <span className="text-sm font-medium text-slate-900 dark:text-white">Valid</span>
              ) : (
                <span className="text-sm font-medium text-red-500">Tampered</span>
              )}
            </div>
          </div>
          ...
          {record.blockchainHash && (
            <div className="mt-4 space-y-2">
              <p className="text-xs text-slate-500 dark:text-neutral-400 font-medium uppercase tracking-wider">Blockchain Hash</p>
              <p className="text-xs font-mono bg-slate-50 dark:bg-neutral-800 px-3 py-2 rounded-lg break-all text-slate-700 dark:text-slate-300">
                {record.blockchainHash}
              </p>
              <a
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl p-6 w-full max-w-md mx-4 animate-slideUp space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-200 dark:border-neutral-800 p-2.5 rounded-lg bg-white dark:bg-neutral-900 text-slate-900 dark:text-slate-100"
              />
            </div>
            <div>
